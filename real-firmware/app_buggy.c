/* app_buggy.c — Deliberate defect: HAL_UART_Transmit commented out in timer callback.
   This simulates a firmware bug where the developer forgot to transmit data. */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    if (htim->Instance == 0x40000400) {
        ...
        /* BUG: HAL_UART_Transmit commented out — no data transmitted */
        /* Expected: "TICK\n" sent every 10 overflows */
        /* Actual:   nothing transmitted */
        /*
        if (timer_ticks % 10 == 0) {
            uint8_t msg[] = "TICK\n";
