...
enum Commands {
    ...
    /// Generate GitHub Actions CI workflow for firmware regression testing
    CiInit,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::CiInit => {
            ...
            let workflow_path = workflows_dir.join("embersim.yml");

            let workflow = r#"# EmberSim Firmware Regression CI
# Generated by 'embersim ci-init'
...
"#;
            ...
            if workflow_path.exists() {
                println!("CI workflow already exists at {}", workflow_path.display());
                println!("Remove it first to regenerate, or edit it manually.");
            } else {
